...
class TeamService:
    def __init__(self, companies: CompanyRepository, teams: TeamRepository, memberships: MembershipRepository):
    ...
    async def list_by_company(self, company_id: UUID, *, only_active: bool = True, limit: int = 100, offset: int = 0):
        company = await self._companies.get(company_id)
        if not company:
            raise NotFound("Компания не найдена")
